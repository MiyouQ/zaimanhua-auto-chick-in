...
TOPIC_ID = 601
TOPIC_PAGE = "https://zt.zaimanhua.com/details?id=601"
SIGN_KEY = "v2hkj_3K5Ly!Ht$"
CHANNEL = "h5"
...
ACTIVITY_END = datetime(2026, 9, 1, 0, 0, 0, tzinfo=BEIJING_TZ)
...
def is_activity_expired():
    ...
    if now >= ACTIVITY_END:
        print(f"[SKIP] 当前北京时间 {now:%Y-%m-%d %H:%M:%S} 已过活动结束时间 "
              f"2026-08-31 24:00，自动跳过，不登录账号、不执行任务")
        return True
    print(f"当前北京时间 {now:%Y-%m-%d %H:%M:%S}，活动进行中 "
          f"(截止 2026-08-31 24:00)")
    return False
...
def build_headers(token):
    return {
        ...
        "Referer": "https://activity.zaimanhua.com/school-season/",
...
def run_read_task(cookie_str):
    ...
    if not token:
        ...
        return False
    return SeasonReader(token).read()
...
def main():
    ...
    print(f"\n{'=' * 60}")
    if all_success:
        print("所有账号 school-season 活动执行完成！")
    else:
