...
CHECKIN_TASK_ID = 8
VIP_TASK_ID = 16
SIGN_IN_URL = "https://m.zaimanhua.com/lpi/v1/task/sign_in"
...
def checkin_via_api(token):
    headers = {
        'Authorization': f'Bearer {token}',
        'Content-Type': 'application/json',
    }

    resp = requests.post(SIGN_IN_URL, headers=headers, timeout=15)
    result = resp.json()

    if result.get('errno') == 0:
        print("签到成功！")
        return True, "签到成功"
    elif result.get('errno') == 1:
        msg = result.get('errmsg', '今天已签到过~')
        print(f"已签到: {msg}")
        return True, msg
    else:
        msg = result.get('errmsg', '未知错误')
        print(f"签到失败: {msg}")
        return False, msg
...
def checkin(cookie_str):
    ...
    if not token:
        print("无法获取 token，签到失败")
        return False

    for attempt in range(1, MAX_RETRIES + 1):
        print(f"尝试第 {attempt}/{MAX_RETRIES} 次...")
        try:
            success, msg = checkin_via_api(token)
            if success:
                return True
